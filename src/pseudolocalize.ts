...
export default function pseudoLocalize(
  string: string,
  {
    delimiter,
  ...
  const pseudolocalized = parts.reduce<string>((pseudolocalized, part) => {
    const pseudolocalizedPart =
      typeof part === 'string'
        ? [...part]
            .map(character => {
    ...
    return pseudolocalized + pseudolocalizedPart;
  }, '');

  return `${prepend || ''}${pseudolocalized}${append || ''}`;
}
...
function createParts(
  string: string,
  {
    startDelimiter,
    endDelimiter,
  }: Pick<Options, 'startDelimiter' | 'endDelimiter'>,
) {
